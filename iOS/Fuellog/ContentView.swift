...
struct ContentView: View {
    ...
    @State private var editingEntry: FuellogEntry?
                        ...
                        ForEach(store.entries) { entry in
                            Button {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    if !entry.gallons.isEmpty {
                                        Text("Gallons: \(entry.gallons)")
                                            .font(.caption)
                                            .foregroundStyle(Theme.accent)
                                    }
                                }
...
enum EntryFormMode {
    case add
    case edit(FuellogEntry)
}

struct EntryFormView: View {
    let mode: EntryFormMode
    var onSave: (FuellogEntry) -> Void
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Gallons", text: $field2)
                        .accessibilityIdentifier("field2Field")
                        .focused($focusedField, equals: .field2)
                    TextField("Price / Gallon", text: $field3)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry: FuellogEntry
                        if case .edit(let existing) = mode {
                            entry = existing
                        } else {
                            entry = FuellogEntry(title: "")
                        }
                        entry.title = title.isEmpty ? "Untitled" : title
                        entry.date = date
                        entry.gallons = field2
                        entry.pricePerGallon = field3
                        entry.note = note
                        onSave(entry)
                        dismiss()
                    }
            ...
            .onAppear {
                if case .edit(let existing) = mode {
                    ...
                    date = existing.date
                    field2 = existing.gallons
                    field3 = existing.pricePerGallon
                    note = existing.note
                }
            }
        }
